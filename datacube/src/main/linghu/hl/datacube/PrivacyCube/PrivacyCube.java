...
public class PrivacyCube {
    ...
    public ArrayList equivalentList = new ArrayList<Integer>();

    public PrivacyCube(int dimension) {
        this(new ArrayList<>(), dimension);
    }

    public PrivacyCube(List<Tuple2<String, List<Object>>> schemaMap, int dimension) {
    ...
    public WebNode getWebTree(List<Integer> schema, List<Integer> sensitives, DistanceMeasure distanceMeasure) {
        ...
        // 先构造搜索路径
        // 对于(3,1)的schema查询，(0,0,0,1)和(0,1,0,0)都是需要的，为了方便合并操作
        for (int i = 0; i<schema.size(); i++){
            List<Set<Integer>> query = new ArrayList<>();
            query.add(Set.of(1));
            queries.set(schema.get(i), query);
            updateCuboid(queries);
            queries.set(schema.get(i), new ArrayList<>());
        }
        // 对于(3,1)的schema查询，(0,0,0,1)和(0,1,0,1)都是需要的，因为要展示
        for (int i = 0;i<schema.size();i++){
            List<Set<Integer>> query = new ArrayList<>();
            query.add(Set.of(1));
            queries.set(schema.get(i), query);
    ...
    public WebNode highlightWebTree(List<Integer> schema, List<Integer> sensitives, List<Set<Integer>> labels,
        ...
        return dfsHighlightWebTree(schema, path, sensitives, labels, globals, distanceMeasure);
    }
        ...
        for (int i = 0; i < schemaMap.size(); i++) {
            ...
            for (int j = 0; j < schema.size(); j++) {
                ...
                if (schema.get(j) == i && path.size() - 1 > j) {
                    ...
                    // !!!!queries在updateCuboid的时候只有有和无的区别，但在query查询时有标签区别!!!!
                    queries.add(query);
                    flag = false;
                }
            }
...
//            }
//        }
        // 以下功能是创建对应标签的结点，并在自我调用中根据path和schema共同决定的query获得content，赋给child
        // 但这个无法完成合并操作
        /*
        for (int i=0; i< schemaMap.get(schema.get(path.size()-1)).y.size();i++){
            WebNode child = new WebNode();
            child.labels = Set.of(i);
            path.push(child);
            child = dfsWebTree(schema, path, sensitives, globals, distanceMeasure);
            path.pop();
            if (child != null) {
    ...
    private WebNode dfsHighlightWebTree(List<Integer> schema, Stack<WebNode> path, List<Integer> sensitives,
                                        List<Set<Integer>> labels, List<double[]> globals, DistanceMeasure distanceMeasure) {
        WebNode root = path.peek(); // 获取栈顶元素
        List<List<Set<Integer>>> queries = new ArrayList<>();
        for (int i = 0; i < schemaMap.size(); i++) {
            boolean flag = true;
        ...
        // 以下的作用仅是规定查找标签
        // 对于前k-1个标签只查找对应的，对于第k个标签查全
        if (path.size() < schema.size() && ((ArrayList)labels.get(path.size()-1)).size() != 0) {
            Set<Integer> label = new HashSet<>(labels.get(path.size() - 1));
            PNode childNode = node.getChild(label);
            WebNode child = new WebNode();
            child.labels = childNode.getLabel();
            path.push(child);
            child = dfsHighlightWebTree(schema, path, sensitives, labels, globals, distanceMeasure);
            path.pop();
            if (child != null) {
                root.children.add(child);
            }
            ...
            for(Map.Entry<Set<Integer>, List<IContent>> entry : nchildren.entrySet()) {
                ...
                if(child==null)
                    if(children.size()==1)
                        ...
                        child = (PNode) node.addChildNode(new PNode(key), false); // 注意这个标签，要是key
...
//                System.out.println("????");
    ...
    public double calculate(WebNode root) {
        equivalentList.clear();
        calculateLeaf(root);
        // todo 计算
        double sumMolecal = 0;
        for (Object k: equivalentList ){
            sumMolecal = sumMolecal + Math.log10((int)k);
        }
        return 1 - sumMolecal/ (equivalentList.size() * Math.log10(root.count));

    }

    public void calculateLeaf(WebNode root){
        if (root.children.size() == 0){
            equivalentList.add(root.content.k);
        }
        else{
