<template>
  <!-- @click.ctrl是按着Ctrl建点击，触发的是那一行选灰的功能，在class中为两者设置了样式区别 -->
  <div class="d-flex data-row" @click.ctrl="rowClick" :class="{'could-drop': couldDrop, 'data-row-selected': selected}">
    ...
    -->
    <!-- schema的格式是s:属性的编号，index:在schema中的序列号   -->
...
import NumberGroupStock from "../DataStock/Number/NumberGroupStock";
import CategoricalGroupStock from "../DataStock/Categorical/CategoricalGroupStock";
...
export default {
  ...
  methods: {
    rowClick(){
      if(this.selected) {
        this.selectedRemove(this.rDataGroup)  // 修改selectRow的
      } else {
    ...
    checkDrag(index){
      ...
      if(index === this.path.length - 1) {
        // 此时的this.path是移动后地方的path，要一致才能合并，一致的含义是前length-1个一致，最后一个不同
    ...
    onDrop(event){
      ...
      this.$emit("mergeGroup", this.$global.dragged, this.path) // 触发外组件的mergeGroup，并传参
    ...
    onDragStart(event, index){
      // path是合并条目的地方 如有四个属性，在第三个属性那里收齐来，然后拖拽的时候path是[1,0,1]，其中元素值是在那个属性中的属性值
      // event是schema中的某个，如果index不是合并path的最后一个，就阻止；
      // 否则，将合并拖拽者设置为此时的path
      if(index !== this.path.length - 1) {
